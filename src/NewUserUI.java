...
public class NewUserUI extends JFrame implements ActionListener 
{
	...
	public NewUserUI(){
		...
		setIconImage(getToolkit().getImage("D:\\SRU ACADEMIC\\SRU SEM-6\\Mini Project\\Keystroke-Biometrics-Identification-System-master\\Keystroke Identification\\Images\\NotePad.gif"));
		setResizable(false);
		setLayout(null);
			...
			if(userNameField.getText().trim().equals("") || passField.getText().trim().equals("") ||
				...
				if(DBUtility.findByUsername(userNameField.getText())== null){
					...
					User user = new User(userNameField.getText(),passField.getText());
 					...
 					boolean added1 = DBUtility.createTable(user);
					if(added&&added1){
						JOptionPane.showMessageDialog(null, "Registered! Now, you will enter the training mode.\nSo, you must enter your login credentails "+Constants.NUMBER_OF_TRAINING_TRIALS+" times");
						setVisible(false);
						dispose();
						new LearningUI(user);
					}else {
